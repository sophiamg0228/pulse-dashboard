...
async function main() {
  ...
  // Parse total_value insights
  const im = {};
  (totals.data || []).forEach(d => { im[d.name] = d.total_value?.value || 0; });

  // 28-day reach (último valor de la serie)
  let reach28 = 0;
  ...
  console.log('✓ metrics.json generado');
  console.log(`  Seguidores: ${output.ig.followers}`);
  console.log(`  Views 30d: ${output.ig.views_30d}`);
  console.log(`  Alcance 28d: ${output.ig.reach_28d}`);
  console.log(`  Interacciones 30d: ${output.ig.interactions_30d}`);
}
